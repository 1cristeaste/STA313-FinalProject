...
There are some limitations that are important to keep in mind when viewing the viewing the visualizations. For the visualization that examines which countries emit plastic into the ocean, the scales for the left and right x-axes are different, while a reader may make an initial assumption that they are the same and the values are comparable. The second visualization normalizes the imports and exports so that they all fit on the graph, but this means the y-axis has no relevant meaning and does not provide information on the amounts that are produced and exported. Further, there are so many countries that do not produce or export much plastic that the lines at the bottom of the graph are difficult to distinguish.
